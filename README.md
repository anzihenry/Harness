...
当前仓库提供的是一个零依赖 Node.js CLI，加上一套可扩展的资产模型与适配层。当前仓库基线版本是 `0.2.0`，正在推进 `0.3.0` 的资产维护、依赖治理和发布物强化能力。
...
`0.3.0` 已经按 Phase 推进资产维护、依赖图治理和发布物强化。接下来建议继续收口发布准备：

1. 增加完整 walkthrough。
2. 增加 `0.3.0` release prep 文档。
3. 在 `CHANGELOG.md` 预留 `0.3.0 - Unreleased`。
